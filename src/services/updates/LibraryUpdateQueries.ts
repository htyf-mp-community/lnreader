  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (cover) {
      ...
      downloadFile(
        cover,
        novelCoverPath,
        (await getPlugin(pluginId))?.imageRequestInit,
      );
      cover = novelCoverUri + '?' + Date.now();
    }
    db.withTransactionSync(async () => {
      try {
        await db.runAsync(`UPDATE Novel SET 
...
const updateNovelTotalPages = (novelId: number, totalPages: number) => {
  return new Promise((resolve, reject) => {
    db.withTransactionSync(async () => {
      try {
        await db.runAsync('UPDATE Novel SET totalPages = ? WHERE id = ?', totalPages, novelId)
        resolve(null)
      } catch (error) {
        reject(error);
      }
    ...
    db.withTransactionSync(async () => {
      for (let position = 0; position < chapters.length; position++) {
        ...
        try {
          ...
          const insertId = data.lastInsertRowId;
